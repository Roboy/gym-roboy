...
class RoboyEnv(gym.GoalEnv):
    ...
    def step(self, action):
        ...
        self._last_state = self._ros_proxy.forward_step_command(action)
        self._current_step_num += 1
        obs = self._make_obs(robot_state=self._last_state)
        info = {}
        reward = self.compute_reward(current_state=self._last_state, goal_state=self._goal_state, info=info)
        done = self._did_complete_successfully(current_state=self._last_state, goal_state=self._goal_state) or \
               self._current_step_num > self._MAX_EPISODE_LENGTH
        if done:
            self._set_new_goal()
